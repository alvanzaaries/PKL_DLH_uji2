    ...
    <div class="bg-white dark:bg-surface-dark shadow rounded-lg overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm py-2 px-3 border" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Email</label>
                <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm py-2 px-3 border" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Role</label>
                <p class="py-2 px-3 bg-gray-50 dark:bg-gray-800 rounded-md text-sm text-gray-700 dark:text-gray-200">
                ...
                @if ($user->id === auth()->id())
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Anda sedang mengedit akun yang sedang login.</p>
                @endif
            </div>
            ...
            <div class="border-t border-gray-100 dark:border-gray-700 pt-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <input type="password" name="password" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm py-2 px-3 border">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Konfirmasi Password</label>
                        <input type="password" name="password_confirmation" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm py-2 px-3 border">
                    </div>
                </div>

                <div class="mt-4">
                    <button form="resetPasswordForm" type="submit" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700" onclick="return confirm('Reset password untuk user ini?');">
                        Reset Password
                    </button>
                </div>
            </div>
