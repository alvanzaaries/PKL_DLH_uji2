...
function updateActiveFilterCount(filterParams = []) {
    ...
    if (countElement) {
        if (count > 0) {
            ...
            countElement.style.color = '#15803d';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const hasActiveFilters = Array.from(params.keys()).some(key => 
        ['nama', 'search', 'kabupaten', 'jenis_produksi', 'kapasitas', 
         'pemberi_izin', 'sumber_bahan_baku', 'bulan', 'tahun', 'status'].includes(key)
    );
    
    if (hasActiveFilters) {
        const filterBody = document.getElementById('filterBody');
        if (filterBody && !filterBody.classList.contains('show')) {
            toggleFilter();
        }
    }
});
