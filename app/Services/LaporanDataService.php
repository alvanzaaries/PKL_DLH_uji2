...
class LaporanDataService
{
    ...
    /**
     * Upload multiple laporan files sekaligus
     */
    public function storeMultipleLaporan($request, $validationService)
    {
        $uploaded = 0;
        $errors = [];
        $skipped = [];
        ...
        for ($i = 0; $i < count($jenisLaporan); $i++) {
            ...
            try {
                ...
                $previewData = $validationService->readAndValidateExcel($filePath, $jenis);
        ...
        return [
            'uploaded' => $uploaded,
            'message' => $message ?: 'Tidak ada laporan yang diupload.'
        ];
    }
